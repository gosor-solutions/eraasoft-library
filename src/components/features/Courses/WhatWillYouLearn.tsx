      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {learn.map((item, index) => (
            <LearnCard key={index} index={index + 1} item={item} />
          ))}
        </div>
      </div>
    </section>
...
function LearnCard({ index, item }: { index: number; item: string }) {
  return (
    <div
      key={index}
    ...
    >
      ...
      <div className="absolute -right-4 -top-20 text-[220px] font-extrabold text-brand-primary/10 ">
        {index}
      </div>
      ...
      <div className="relative z-10 min-h-24 flex flex-col justify-center">
        {/* Description */}
        <div className="mb-6 space-y-2 max-w-3/4">{item}</div>
      </div>
    </div>
  );
}
